...
def check_doi_handle(doi):
    ...
    try:
        ...
        code = resp.getcode()
        return (code in [200, 301, 302, 303, 307, 308]), code
...
def process_item(ch, num, text, raw_html):
    existing_doi = None
    ...
    if existing_doi:
        handle_ok, code = check_doi_handle(existing_doi)
        meta = fetch_crossref_doi_metadata(existing_doi)
        if handle_ok and meta.get("valid"):
            sim = title_similarity(text, meta["title"])
            if sim >= 0.60:
                status = "VALID_EXISTING"
                final_doi = meta["doi"]
                notes = f"DOI Handle {code} OK, title match {sim:.2f}: '{meta['title']}'"
            else:
                status = "HALLUCINATED"
                notes = f"Existing DOI points to WRONG paper '{meta['title']}' (sim {sim:.2f})"
        else:
            status = "BROKEN_HTTP"
            notes = f"Existing DOI Handle status: {code}, CrossRef valid: {meta.get('valid')}"
    ...
    if status != "VALID_EXISTING":
        ...
        if best_cand and best_sim >= 0.70:
            ...
            handle_ok, code = check_doi_handle(cand_doi)
            if handle_ok:
                final_doi = cand_doi
                ...
                notes += f" | Candidate DOI Handle status {code}"
        else:
            if status == "HALLUCINATED":
                ...
                status = "NO_DOI"
                notes = "No valid DOI found (book, website, or unindexed)"
                
    return {
        "chapter": f"chapter_{ch:02d}",
        "num": num,
        "text": text,
...
for ch in range(8, 15):
    ...
    if ch == 9:
        for idx, text in enumerate(ch9_refs_docx):
            all_tasks.append((ch, idx+1, text, f"<li>{text}</li>"))
    elif ch == 11:
        ...
        for idx, p in enumerate(num_p):
            all_tasks.append((ch, idx+1, p.get_text().strip(), str(p)))
    else:
        with open(filepath, 'r', encoding='utf-8') as f:
            soup = BeautifulSoup(f.read(), 'html.parser')
        ...
        for idx, li in enumerate(lis):
            all_tasks.append((ch, idx+1, li.get_text().strip(), str(li)))

print(f"Total reference tasks to process: {len(all_tasks)}")

results_by_ch = {}
with ThreadPoolExecutor(max_workers=10) as executor:
    futures = [executor.submit(process_item, ch, num, text, raw_html) for ch, num, text, raw_html in all_tasks]
    for future in as_completed(futures):
        res = future.result()
        ch_name = res["chapter"]
        if ch_name not in results_by_ch:
            results_by_ch[ch_name] = []
        results_by_ch[ch_name].append(res)

for ch_name in results_by_ch:
    results_by_ch[ch_name].sort(key=lambda x: x["num"])

with open(".agents/teamwork_preview_worker_m2_2_gen2/doi_audit_results_final.json", "w", encoding="utf-8") as f:
    json.dump(results_by_ch, f, indent=2)

print("Final DOI Verification Complete! Saved to .agents/teamwork_preview_worker_m2_2_gen2/doi_audit_results_final.json")
