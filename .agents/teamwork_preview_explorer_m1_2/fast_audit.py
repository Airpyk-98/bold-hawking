...
def query_crossref_search(text):
    ...
    url = f"https://api.crossref.org/works?query.bibliographic={urllib.parse.quote(clean_text[:200])}&rows=1"
    req = urllib.request.Request(url, headers=crossref_headers)
    try:
        with urllib.request.urlopen(req, timeout=5) as resp:
            data = json.loads(resp.read().decode('utf-8'))
...
for ch_file in chapter_files:
    ...
    if heading_container:
        ...
        if next_ol:
            ...
            for li in next_ol.find_all("li", recursive=False):
                ref_items.append({"raw_html": str(li), "text": li.get_text(strip=True), "element": li})
        else:
            ...
            while curr:
                if curr.name == "p":
                    ...
                    if re.match(r'^\d+\.\s', txt):
                        p_items.append({"raw_html": str(curr), "text": txt, "element": curr})
                    elif not txt:
                        pass
                    else:
                        break
    ...
    print(f"Fast Auditing {ch_file}: {len(ref_items)} refs ({container_type})")
    ...
    for idx, item in enumerate(ref_items, 1):
        ...
        anchor_info = [{"href": a.get("href", ""), "text": a.get_text(strip=True), "raw": str(a)} for a in a_tags]
        ...
        if existing_doi:
            ...
            if not cr_doi_info.get("valid"):
                ...
                w_doi_title = [w.lower() for w in re.findall(r'\b[A-Za-z]{4,}\b', cr_title)]
                w_ref_text = [w.lower() for w in re.findall(r'\b[A-Za-z]{4,}\b', text)]
                stop = {"journal", "study", "analysis", "review", "research", "effect", "effects", "human", "using", "between", "characterization"}
                common = set(w_doi_title).intersection(set(w_ref_text)) - stop
                
                if len(w_doi_title) > 3 and len(common) < 2:
